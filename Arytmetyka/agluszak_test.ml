...
assert ((min_wartosc n) = (0.));
assert ((max_wartosc n) = (infinity));
assert (is_nan(sr_wartosc n));

assert ((min_wartosc o, max_wartosc o) = (neg_infinity, 0.));
...
assert ((min_wartosc t, max_wartosc t) = (neg_infinity, 0.));;
